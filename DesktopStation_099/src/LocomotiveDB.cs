...
namespace DesktopStation
{
    public class LocomotiveDB
    {
        ...
        public String GenerateRegCode(int inIndex)
        {
            String aRegCode = "A ";

            if (Items.Count > inIndex)
            {
                LocData aItem = Items[inIndex];

                aRegCode = aRegCode + "LOC" + aItem.mLocAddr + ";";
                aRegCode = aRegCode + "STP" + aItem.mLocSpeedstep + ";";
                aRegCode = aRegCode + "NAM" + aItem.mLocName + ";";
                aRegCode = aRegCode + "MAN" + aItem.mLocManufacture + ";";
                aRegCode = aRegCode + "LDB" + aItem.mLocAddr_dbl + ";";
                aRegCode = aRegCode + "MXS" + aItem.mLocMaxSpeed + ";";
                aRegCode = aRegCode + "ACR" + aItem.mSpeedAccRatio + ";";
                aRegCode = aRegCode + "RER" + aItem.mSpeedRedRatio + ";";

                for (int i = 0; i < 29; i++)
                {
                    if (aItem.mFunctionImageTable[i] > 0)
                    {
                        aRegCode = aRegCode + "F" + i.ToString() + ".I" + aItem.mFunctionImageTable[i] + ";";
                    }
                }
        ...
        public void LoadFromFile(String inFileName)
        {
            ...
            if (File.Exists(inFileName))
            {
                ...
                while (aParser.EndOfData == false)
                {
                    ...
                    aItem.mLocItemNo = GetCSVFieldString(aFields, 3, "");
